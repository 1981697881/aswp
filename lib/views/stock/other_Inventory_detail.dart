...
class _OtherInventoryDetailState extends State<OtherInventoryDetail> {
    ...
    if(stockNumber == null || stockNumber == ""){
      ...
      if (fBarCodeList == 1) {
        ...
        for(var item in barcodeList){
          ...
          barcodeMap['FormId'] = 'QDEP_Cust_BarCodeList';
          barcodeMap['FieldKeys'] =
          'FID,FInQtyTotal,FOutQtyTotal,FEntity_FEntryId,FRemainQty,FBarCodeQty,FStockID.FName,FStockID.FNumber,FMATERIALID.FNUMBER,FOwnerID.FNumber,FBarCode,FSN,FPackageSpec,FProduceDate,FExpiryDate,FStockLocNumberH,FStockID.FIsOpenLocation,FBatchNo,FMATERIALID.FIsKFPeriod,FMATERIALID.FIsBatchManage';
          Map<String, dynamic> dataMap = Map();
          dataMap['data'] = barcodeMap;
          String order = await CurrencyEntity.polling(dataMap);
          var barcodeData = jsonDecode(order);
  ...
  getMaterialList(barcodeData,code, fLoc,fIsOpenLocation,fAuxPropId,fProduceDate,fExpiryDate) async {
    ...
    userMap['FieldKeys'] =
    'FStockOrgId.FNumber,FMaterialId.FName,FMaterialId.FNumber,FMaterialId.FSpecification,FBaseUnitId.FName,FBaseUnitId.FNumber,FStockId.FNumber,FBaseQty,FStockName,FLot.FNumber,FStockStatusId.FNumber,FKeeperTypeId,FKeeperId.FNumber,FOwnerId.FNumber,FStockLocId.'+flexNumber+'.FNumber,FStockLocId.'+flexNumber+'.FNumber,FStockLocId.'+flexNumber+'.FNumber,FStockLocId.'+flexNumber+'.FNumber,FStockID.FIsOpenLocation,FAuxPropId,FMaterialId.FIsKFPeriod,FStockID.FIsOpenLocation,FMaterialId.FIsBatchManage,FProduceDate,FExpiryDate';
    Map<String, dynamic> dataMap = Map();
    dataMap['data'] = userMap;
    String order = await CurrencyEntity.polling(dataMap);
    orderDate = [];
