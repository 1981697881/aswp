...
class _ReturnGoodsDetailState extends State<PurchaseReturnDetail> {
      ...
      //orderDate.forEach((value) {
        ...
        arr.add({
          ...
          "value": {
            ...
            "barcode": [],
            "kingDeeCode": [],
            "surplus": (value[12] - value[14])>0?(value[12]*100 - value[14]*100)/100: 0,
            "scanCode": []
          }
        });
        arr.add({
       ...
       /* Map<String, dynamic> inventoryMap = Map();
        ...
        arr.add({
          ...
          "name": "",
          "isHide": false,
          "value": {"label": (value[12] - value[14])>0?(value[12]*100 - value[14]*100)/100: 0, "value": (value[12] - value[14])>0?(value[12]*100 - value[14]*100)/100: 0, "rateValue": (value[12] - value[14])>0?(value[12]*100 - value[14]*100)/100: 0}
        });
        arr.add({
          "title": "最后扫描数量",
          "name": "FLastQty",
  ...
  getMaterialList(barcodeData,code, fsn, fProduceDate, fExpiryDate, fLoc,fIsOpenLocation) async {
    ...
    userMap['FieldKeys'] =
    'FMATERIALID,FName,FNumber,FSpecification,FBaseUnitId.FName,FBaseUnitId.FNumber,FIsBatchManage,FIsKFPeriod';/*,SubHeadEntity1.FStoreUnitID.FNumber*/
    Map<String, dynamic> dataMap = Map();
    dataMap['data'] = userMap;
    String order = await CurrencyEntity.polling(dataMap);
    materialDate = [];
          ...
          for (var value in materialDate) {
            ...
            arr.add({
              ...
              "value": {
                ...
                "value": inserNum.toString(),"remainder": ((double.parse(barCodeScan[4])*100 - inserNum*100)/100).toString(),"representativeQuantity": barCodeScan[4]
              }
            });
            arr.add({
              "title": "包装数量",
        ...
        if(number == 0){
          ...
          for (var value in materialDate) {
            ...
            arr.add({
              ...
              "value": {
                "label": inserNum.toString(),
                "value": inserNum.toString(),"remainder": "0","representativeQuantity": "0"
              }
            });
            arr.add({
              "title": "包装数量",
